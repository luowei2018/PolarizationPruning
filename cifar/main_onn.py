...
def train(epoch, available_ratio, weight_valid_mask=None):
    model.train()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        separation = batch_idx%len(available_ratio)
        if args.cuda:
        ...
        if args.loss in {LossType.ONESHOT, LossType.PROGRESSIVE_SHRINKING}:
            ...
            weight_valid_mask = create_mask(model, separation)
        ...
        if args.loss in {LossType.INTERSECT}:
            weight_valid_mask = create_mask(model, available_ratio[separation])
            freeze_mask, dynamic_model, ch_indices = sample_network_dynamic(model, weight_valid_mask, separation)
            output = dynamic_model(data)
        ...
        if args.loss in {LossType.L1_SPARSITY_REGULARIZATION}:
            updateBN()
        if args.loss in {LossType.PROGRESSIVE_SHRINKING, LossType.INTERSECT}:
...
while len(available_ratio) < 10:
    print(len(available_ratio))
    best_prec1 = 0.
    available_ratio.append(ratios[len(available_ratio)])
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(epoch, available_ratio)
        with open(os.path.join(args.save + 'Overall_Accuracy'), 'a+') as fp:
            fp.write(f'\n')
        for idx, rat in enumerate(available_ratio):
            weight_valid_mask = create_mask(model, rat)
            dynamic_model = sample_network_dynamic(model, weight_valid_mask, idx, eval=True)
            if rat == 1:
                print("Epoch accuracy original model: " + str(test(dynamic_model)))
                if not os.path.exists(args.save + '{:.2f}/'.format(rat)):
                    os.makedirs(args.save + '{:.2f}/'.format(rat))
                with open(os.path.join(args.save + '{:.2f}/'.format(rat) + 'Prec'), 'a+') as fp:
                    fp.write(f'{epoch} {test(dynamic_model)}\n')
                    prec_original = test(dynamic_model)
            prec1 = test(dynamic_model)
            print(f"pruned model prec1 :{prec1:.2f}")
            with open(os.path.join(args.save + 'Overall_Accuracy'), 'a+') as fp:
                fp.write(f'{epoch} {rat} {test(dynamic_model)}\n')

        is_best_orginal = prec_original > best_prec_original
        is_best = prec1 > best_prec1
        best_prec1 = max(prec1, best_prec1)
        best_prec_original = max(prec_original, best_prec_original)
        save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
            'best_prec1': best_prec1,
            'best_prec_original': best_prec_original,
            'optimizer': optimizer.state_dict(),
        }, is_best_orginal, filepath=args.save,
            backup_path=args.backup_path,
            backup=epoch % args.backup_freq == 0,
            epoch=epoch,
            max_backup=args.max_backup
        )
    print("Epoch accuracy: " + str(best_prec1) + " " + str(prec1))

print("Best accuracy: " + str(best_prec1))
with open(os.path.join(args.save, 'Overall_Prec_Record.txt'), 'a+') as fp:
    fp.write(f'{weight_valid_mask.float().mean():.5f} {best_prec1}\n')
